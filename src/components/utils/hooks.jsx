...
export const formatAllocation = (allocation) => {
  if (allocation >= 1_000_000) {
    return `${(allocation / 1_000_000).toFixed(1)}M`; // Convert to millions
  } else if (allocation >= 1_000) {
    return `${(allocation / 1_000).toFixed(1)}K`; // Convert to thousands
  }
  return allocation.toString(); // Return as-is if less than 1K
};
